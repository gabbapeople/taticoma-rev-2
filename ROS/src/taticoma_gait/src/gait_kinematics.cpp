...
bool GaitKinematics::init() {
    ...
    if (!node.getParam("robot_description", robot_desc_string)) {
        ROS_ERROR("Node Error : taticoma_gait_kinematics : Could not load the xml from parameter: robot_description");
        return false;
    }
    ...
    if (!loadModel(robot_desc_string)) {
        ROS_ERROR("Node Error : taticoma_gait_kinematics : Could not load models");
        return false;
    }
    ...
    ROS_WARN("Node Ready : taticoma_gait_kinematics");
...
bool GaitKinematics::callService(KDL::Vector* vector) {
    ...
    if (client.call(srv)) {
        if (srv.response.error_codes == srv.response.IK_FOUND) {
            ...
            ROS_ERROR("Node Error : taticoma_gait_kinematics : An IK solution could not be found");
            return 0;
        }
    } else {
        ROS_ERROR("Node Error : taticoma_gait_kinematics : Failed to call service");
        return 0;
    }
    return true;
}
...
bool GaitKinematics::loadModel(const std::string xml) {
    ...
    if (!kdl_parser::treeFromString(xml, tree)) {
        ROS_ERROR("Node Error : taticoma_gait_kinematics : Could not initialize tree object");
        return false;
    }

    ROS_WARN("Node Msg   : taticoma_gait_kinematics : Construct tree");
    ...
    ROS_WARN("Node Msg   : taticoma_gait_kinematics : Get frames");
...
int main(int argc, char** argv) {
    ...
    GaitKinematics g;
    if (g.init() < 0) {
        ROS_ERROR("Node Error : taticoma_gait_kinematics : Could not initialize gait node");
        return -1;
    }
    g.gaitGenerator();
